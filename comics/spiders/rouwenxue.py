...
class RouwenxueSpider(scrapy.Spider):
    '''
    classdocs
    '''
    name = 'rouwenxue';
    allowed_domains = ['rouwenxue.com'];
    ...
    def parse(self, response):
        ...
        title = sel.xpath('//h1/a/span/text()').extract()[0]
        ...
        dd = sel.xpath('//tr[@align="center"]')
        id = 0;        
        for d in dd:
            if len(d.xpath('td/a')) == 0:
                continue;
            id += 1;
            url = d.xpath('td/a/@href').extract()[0];
            url = response.urljoin(url);
            
            subtitle = d.xpath('td/a/text()').extract()[0];
    ...
    def parse_page(self, response):
        ...
        sel = Selector(response);
        content = sel.xpath('//div[@class="novel_content"]/text()').extract();
